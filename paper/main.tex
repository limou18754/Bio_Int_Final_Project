...
Table~\ref{tab:rationale} makes explicit how those parameter changes relate to the biological interpretation of dopamine depletion. To keep the literature support conservative, the table cites only papers that directly motivate the corresponding pathway-level claim. The first three rows therefore cover the main mechanistic hypotheses of the project: weaker direct-pathway influence, stronger indirect-pathway influence, and stronger effective STN-GPe loop gain. The final row groups several smaller recurrent or downstream adjustments that were used phenomenologically to keep the reduced network in a stable parkinsonian low-beta regime, and I do not assign those auxiliary scale factors separate one-to-one literature citations.

\begin{table}[htbp]
  \centering
  \small
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{p{2.8cm}p{3.7cm}p{5.6cm}p{2.8cm}}
    \hline
    Model component & Parkinsonian implementation & Biological rationale in the literature & Key references \\
    \hline
    Direct pathway weakened & D1 external drive scaled to 0.85; D1 $\rightarrow$ GPi weight scaled to 0.72 & The classic rate-model account of parkinsonism treats dopamine depletion as reducing direct-pathway influence while increasing basal ganglia output. The present parameter change is an abstract implementation of that direction of effect. & \citep{Albin1989,DeLong1990} \\
    Indirect pathway strengthened & D2 external drive scaled to 1.15; D2 $\rightarrow$ GPe weight scaled to 1.62 & Dopamine loss increases indirect-pathway influence, and increased striatal inhibition of GPe is a direct control parameter for oscillation generation in Parkinsonian network models. & \citep{DeLong1990,Kumar2011} \\
    Reciprocal STN-GPe loop gain increased & STN $\rightarrow$ GPe weight scaled to 2.10; GPe $\rightarrow$ STN weight scaled to 1.38 & Experimental and modeling work both point to the subthalamo-pallidal loop as a key beta-generating circuit. In the reduced model, these scale factors represent a higher-gain delayed excitatory-inhibitory loop rather than a direct measurement of synaptic conductance change. & \citep{Mallet2008,Holgado2010} \\
    Secondary recurrent and downstream adjustments & GPe $\rightarrow$ GPe scaled to 1.67; STN $\rightarrow$ STN scaled to 3.33; STN $\rightarrow$ GPi scaled to 1.45; GPe $\rightarrow$ GPi scaled to 1.10; STN/GPe/GPi drives slightly increased & These terms help the simplified network maintain coherent low-beta oscillations and elevated downstream output once the main pathway-balance changes are applied. They are included as phenomenological operating-point adjustments, not as individually literature-constrained parameters. & --- \\
    \hline
  \end{tabular}%
  }
  \caption{Biological rationale for the main parkinsonian parameter modifications. The table distinguishes between core hypothesis-driven changes and auxiliary phenomenological adjustments used to place the reduced network in a stable pathological regime.}
  \label{tab:rationale}
\end{table}
